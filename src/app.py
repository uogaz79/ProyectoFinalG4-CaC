...
#Para utilizarlo localmante:
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:Labcas01@localhost/proyecto"
#para utilizarlo con el backend app.py subido al pythonanywhere.com: 
#app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://uogaz:curso2023@uogaz.mysql.pythonanywhere-services.com/uogaz$proyecto"
...
class LineaProducto(db.Model):  # Modificacion UOGAZ 
    """
    Definición de la tabla LineaProducto en la base de datos.
    La clase LineaProducto hereda de db.Model.
    Esta clase representa la tabla "LineaProducto" en la base de datos.
    """
    lineaid = db.Column(db.Integer, primary_key=True) # Modificacion UOGAZ 
    lineadesc = db.Column(db.String(100)) # Modificacion UOGAZ 

    def __init__(self, lineadesc):  # Modificacion UOGAZ 
        """
        Constructor de la clase LineaProducto.

        Args:
            lineadesc (str): Nombre de la Linea
        """
        self.lineadesc = lineadesc
...
class Producto(db.Model):  # Producto hereda de db.Model
    ...
    fechaalta = db.Column(db.DateTime, nullable = False, default= date.today()) # Modificacion UOGAZ 
    lineaid = db.Column(db.Integer, db.ForeignKey(LineaProducto.lineaid), nullable = False) # Modificacion UOGAZ 

    def __init__(self, nombre, precio, stock, imagen, fechaalta, lineaid):  # Modificacion UOGAZ 
    #def __init__(self, nombre, precio, stock, imagen):
        ...
        self.stock = stock
        self.imagen = imagen
        self.fechaalta = fechaalta  # Modificacion UOGAZ 
        self.lineaid = lineaid  # Modificacion UOGAZ 
...
class ProductoSchema(ma.Schema):
    ...
    """
    class Meta:
        fields = ("id", "nombre", "precio", "stock", "imagen", "fechaalta", "lineaid")  # Modificacion UOGAZ 
...
# Definición del esquema para la clase LineaProducto
class LineaProdSchema(ma.Schema):
    """
    Esquema de la clase LineaProducto.

    Este esquema define los campos que serán serializados/deserializados
    para la clase LineaProducto.
    """
    class Meta:
        fields = ("lineaid", "lineadesc")  # Modificacion UOGAZ 

linea_schema = LineaProdSchema()  # Objeto para serializar/deserializar un producto
lineas_schema = LineaProdSchema(many=True)  # Objeto para serializar/deserializar múltiples productos
...
@app.route("/lineasprod", methods=["GET"])
def get_LineasProducto():
    """
    Endpoint para obtener todos los productos de la base de datos.

    Retorna un JSON con todos los registros de la tabla de productos.
    """
    all_lineas = LineaProducto.query.all()  # Obtiene todos los registros de la tabla de productos
    result = lineas_schema.dump(all_lineas)  # Serializa los registros en formato JSON
    return jsonify(result)  # Retorna el JSON de todos los registros de la tabla

'''
El código que sigue a continuación termina de resolver la API de gestión de productos, 
a continuación se destaca los principales detalles de cada endpoint, incluyendo su funcionalidad y el tipo de respuesta que se espera.
...
#"""
@app.route("/lineasprod/<id>", methods=["GET"])
def get_lineasproducto(id):
    """
    Endpoint para obtener un Linea específico de la base de datos.
    Retorna un JSON
    """
    linea = LineaProducto.query.get(id)  # Obtiene el producto correspondiente al ID recibido
    return linea_schema.jsonify(linea)  # Retorna el JSON del producto
...
#"""
@app.route("/lineasprod/<id>", methods=["DELETE"])
def delete_linea(id):
    """
    Endpoint para eliminar un Linea
    Y retorna un JSON con el registro eliminado.
    """
    linea = LineaProducto.query.get(id)  # Obtiene el producto correspondiente al ID recibido
    db.session.delete(linea)  # Elimina el producto de la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return linea_schema.jsonify(linea)  # Retorna el JSON del producto eliminado
#"""
...
def create_producto():
    ...
    fechaalta = request.json["fechaalta"]  # Modificacion UOGAZ 
    lineaid = request.json["lineaid"]  # Modificacion UOGAZ 
    #new_producto = Producto(nombre, precio, stock, imagen)  # Crea un nuevo objeto Producto con los datos proporcionados # Modificacion UOGAZ 
    new_producto = Producto(nombre, precio, stock, imagen, fechaalta, lineaid)  # Modificacion UOGAZ 
...
#"""
@app.route("/lineasprod", methods=["POST"])  # Endpoint para crear una Linea de Producto
def create_linea():
    """
    Endpoint que lee los datos proporcionados en formato JSON por el cliente y crea un nuevo registro de Lineas en la base de datos.
    Retorna un JSON con el nueva Linea creada.
    """
    lineadesc = request.json["lineadesc"]  # Obtiene el nombre Linea del JSON proporcionado
    new_linea = LineaProducto(lineadesc)  # Modificacion UOGAZ 
    db.session.add(new_linea)  # Agrega el nuevo producto a la sesión de la base de datos
    db.session.commit()  # Guarda los cambios en la base de datos
    return linea_schema.jsonify(new_linea)  # Retorna el JSON del nuevo producto creado
#"""

#"""
@app.route("/lineasprod/<id>", methods=["PUT"])  # Endpoint para actualizar una Linea
def update_linea(id):
    """
    Endpoint lee los datos proporcionados en formato JSON por el cliente y actualiza el registro de la Linea con el ID especificado.
    Retorna un JSON con la Linea actualizada.
    """
    linea = Producto.query.get(id)  # Obtiene el producto existente con el ID especificado

    # Actualiza los atributos del producto con los datos proporcionados en el JSON
    linea.lineadesc = request.json["lineadesc"] 
   
    db.session.commit()  # Guarda los cambios en la base de datos
    return linea_schema.jsonify(linea)  # Retorna el JSON del producto actualizado
#"""
...
def update_producto(id):
    ...
    producto.fechaalta = request.json["fechaalta"]  # Modificacion UOGAZ 
    producto.lineaid = request.json["lineaid"]  # Modificacion UOGAZ 
